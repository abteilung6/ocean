package org.abteilung6.ocean
package utils

object Validator {
  private val validEmail =
    """(?:[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*|"(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21\x23-\x5b\x5d-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])*")@(?:(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?|\[(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?|[a-z0-9-]*[a-z0-9]:(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21-\x5a\x53-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])+)\])""".r

  private val alphaNumeric = "^[a-zA-Z0-9]*$".r

  /**
   * General Email Regex (RFC 5322 Official Standard).
   */
  def validateEmail(value: String): Boolean = validEmail.matches(value)

  def isAlphanumeric(value: String): Boolean = alphaNumeric.matches(value)

  def min(value: String, length: Int): Boolean = value.length >= length

  def max(value: String, length: Int): Boolean = value.length <= length
}
